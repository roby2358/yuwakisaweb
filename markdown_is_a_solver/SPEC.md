...
- A `declare` list item introduces one or more constants of a single sort. The bullet's first token is the sort (`Int`, `Real`, `Bool`, or `String`); the remaining tokens on the same line — and any indented sub-bullets — are the names being declared. Example: `* Int alice bob carol` declares three integers.
- Assertions are written as bullet trees where nesting IS the expression tree. The first token of a bullet line is the node's value (operator name for a compound, literal for an atom, or symbol for a bare reference); any further tokens on that same line become flat children of that node (compact form); indented sub-bullets become additional children at deeper levels. Each top-level bullet under `# assert` is one assertion. Indented sub-bullets are children of the parent node, NOT separate assertions — there is no implicit conjunction of sibling bullets at a sub-level. Use an explicit `and` to conjoin. Multiple `assert` sections MAY appear and are all conjoined at check time.
...
- Surface conventions: backtick-wrapped tokens are literals — numbers (`` `443` ``, `` `0.1` ``), booleans (`` `true` ``, `` `false` ``), quoted strings (`` `"hello"` ``), or bare strings (`` `healthy` ``, `` `0.0.0.0/0` ``) that are neither numeric nor boolean. Unquoted tokens are symbol references (bare identifiers, dot-qualified for JSON field access) or operator names (`and`, `or`, `=`, `+`, `<=`, …). Either `*` or `-` MAY be used as the bullet marker.
