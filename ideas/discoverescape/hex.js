...
class Hex {
    ...
    // Hexes on the straight line to another hex, inclusive of both endpoints,
    // via cube lerp + rounding. The epsilon nudge breaks exact-edge ties so a
    // line never flickers between two equally near hexes.
    lineTo(other) {
        const n = this.distance(other);
        if (n === 0) return [new Hex(this.q, this.r)];
        const out = [];
        for (let i = 0; i <= n; i++) {
            const t = i / n;
            out.push(Hex.round(
                this.q + (other.q - this.q) * t + 1e-6,
                this.r + (other.r - this.r) * t + 2e-6));
        }
        return out;
    }
