...
// engine.js — rules resolution: the monthly loop, duels, campaigns, and the
// forecast functions the panels read. The invented town simulation lives in
// town.js. MIT License.
...
function regimentRelation(char, npc) {
  ...
  if (npc.regimentId === char.regimentId) return 'friendly';
  const relations = REGIMENT_RELATIONS[char.regimentId];
  if (relations.friends.indexOf(npc.regimentId) >= 0) return 'friendly';
  if (relations.enemies.indexOf(npc.regimentId) >= 0) return 'enemy';
  return 'neutral';
}
