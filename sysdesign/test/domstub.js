// A DOM small enough to boot LOAD BEARING headlessly.
//
...
function makeDom(html) {
  ...
  function makeElement(tag) {
    const el = {
      ...
      // enough <table> for the scorecard to build itself
      insertRow() {
        const row = makeElement('tr');
        row.insertCell = () => row.appendChild(makeElement('td'));
  ...
  const document = {
    ...
    createTextNode: text => Object.assign(makeElement('#text'), { textContent: text }),
  ...
  return {
    ...
    fire(id, type) {
      ...
      // modal buttons are wired with .onclick because they are re-bound per open
      if (type === 'click' && typeof el.onclick === 'function') {
        el.onclick(event);
